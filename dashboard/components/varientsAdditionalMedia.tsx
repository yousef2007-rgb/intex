  ...
  const handleImageUploadReq = async (e: any) => {
    ...
    try {
      ...
      if (
        ...
        index != undefined &&
        formData.varients != undefined
      ) {
        const newArray = formData.varients?.slice();
        newArray?.splice(index, 1, {
          ...formData.varients[index],
          imagesUrls: [...formData.varients[index].imagesUrls, imageUrl],
        });
        setFormData({ ...formData, varients: newArray });
      } else {
        setVarient({
          ...varientData,
          imagesUrls: varientData?.imagesUrls
            ? [...varientData.imagesUrls, imageUrl]
            : [imageUrl],
        });
      }
    ...
    <div className=" my-5">
      ...
      <div className="flex items-center">
        {selectedImage ? (
          <img
            src={selectedImage}
            alt="Uploaded"
            className="max-w-[100px] rounded-md mr-2"
          />
        ) : (
          ""
        )}
      ...
      <div className="flex flex-wrap">
        {defaultImage && index != undefined && formData.varients != undefined
          ? formData.varients[index]?.imagesUrls?.map(
              (imageUrl: string, i: number) => (
                <div
                  className=" rounded-md m-2 relative w-fit bg-gray-100"
                  key={i}
                >
                  <img
          ...
          : varientData?.imagesUrls?.map((imageUrl: string, index: number) => (
              <div
                className=" rounded-md m-2 relative w-fit bg-gray-100"
                key={index}
              >
